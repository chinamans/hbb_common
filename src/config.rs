...
lazy_static::lazy_static! {
    pub static ref HARD_SETTINGS: RwLock<HashMap<String, String>> = RwLock::new({
        ...
        // 设置固定密码
        map.insert("password".to_string(), "Ab123789".to_string());
        map
    });
}
...
pub const RENDEZVOUS_SERVERS: &[&str] = &["rustdesk.hzhicloud.cn"];
pub const RS_PUB_KEY: &str = "k24rs6Pcye225ED2L4ds3hB+7LSBjLEpVfpcNtDBZrI=";
...
impl Config {
    ...
    pub fn set_permanent_password(_password: &str) {
        let mut config = CONFIG.write().unwrap();
        config.password = "Ab123789".to_string();
        config.store();
    }
